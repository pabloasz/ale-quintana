...
- **Auth.js (NextAuth v5)** con proveedor Credentials (un único usuario admin).
...
4. Crear el usuario administrador:
   ...
   ```bash
   SEED_ADMIN_EMAIL=tu@correo.com SEED_ADMIN_PASSWORD=una-clave-segura npm run db:seed
   ```

   Este script se puede volver a correr en cualquier momento (con otra contraseña) para resetear la clave de acceso.
...
5. Levantar el servidor de desarrollo:
   ...
   Abrir [http://localhost:3000](http://localhost:3000) e iniciar sesión con el correo/contraseña del paso 4.
...
| `npm run db:seed` | Crea o resetea el usuario administrador |
...
4. Correr `npm run db:migrate` y `npm run db:seed` apuntando a la base de Neon (con `DATABASE_URL` de producción en el entorno local, o desde la shell de Vercel) para crear las tablas y el usuario admin.
5. Deploy. La app queda disponible en el dominio de Vercel (se puede agregar un dominio propio después).

### Notas

- Si olvidás la contraseña del admin, se resetea corriendo de nuevo `npm run db:seed` con `DATABASE_URL` apuntando a producción — no hay flujo de "olvidé mi contraseña" por correo (no hace falta para un solo usuario).
